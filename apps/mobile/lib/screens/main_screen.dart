...
class _MainScreenState extends State {
  ...
  Future<void> _attemptSilentReconnect() async {
    ...
    if (!connectedToPort) {
      bool reconnected = await ClientServices().tryAutoReconnect();
      if (reconnected && mounted) {
        setState(() {}); // Rebuild UI to show "Connected"
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Reconnected to previous session!")),
        );
      }
    }
  }
